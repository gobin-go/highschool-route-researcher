...
def _load_korean_font(size=20):
    # 같은 폴더 또는 ./fonts 폴더 모두 탐색
    candidates = [
        Path(__file__).parent / "NotoSansKR-Regular.ttf",
...
def make_legend_image():
    ...
    try:
        draw.rounded_rectangle([1, 1, w-2, h-2], radius=12, outline="#B0B0B0", width=2, fill="#FAFAFA")
    except Exception:
        draw.rectangle([1, 1, w-2, h-2], outline="#B0B0B0", width=2, fill="#FAFAFA")
...
<script>
  ...
  document.getElementById("go").onclick = ()=>{
    ...
    if (startInfo && startInfo.length === 3){
      const startFloor = startInfo[2];
      setFloorRadio(startFloor);
    }
    refresh();
    calcSeq();
  };
